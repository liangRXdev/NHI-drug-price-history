...
**2026-09-16 通過**（臨床藥師目檢，比照 C2）。

環境：`node scripts/upcoming_review.mjs` 同時提供兩個畫面——
**http://127.0.0.1:8766/?view=upcoming** 真實清單（82 列 / 79 代號，資料產生日 2026-09-12）、
**http://127.0.0.1:8767/?view=upcoming** 合成反例（§4.1 的 14 個序號共 18 列，品名即標明該列要驗的規則）。
合成清單在送出前先過 `validateUpcoming()`，不拿一份自己就不合法的資料目檢。

| 項目 | 結果 |
|---|---|
| 1. 無 §4 禁止用語（停產／註銷／下架／藥價黑洞／政策性降價，以及任何調價原因敘述） | ✅ |
| 2. 無「終止／0 元」標籤誤用——`everPriced = false` 的序 4a／4b／4c 三列畫面上**不出現「終止」二字** | ✅ |
| 3. 實際呈現列數等於預期集合（真實 82 列、合成 18 列），非「看起來有資料就算過」 | ✅ |
| 序 5／8 續期顯示停止前金額（終止前 245.00 元／暫停前 18.00 元） | ✅ |
| 序 9 暫停標記顯示來源原值「—」，非寫死的「-」 | ✅ |

## 5. 待辦

- [ ] 合併後手動 dispatch 一次 `Build Price History Data`，確認例行路徑在真實環境仍維持
