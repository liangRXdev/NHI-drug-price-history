...
### `/codex-review` 修正後（2026-09-12，判定見 `.ai-review/verdict.md`）

- 新增：`e2e/real-data.spec.mjs`（讀實際發布的 `data/`，不 mock）——R1「正式資料缺欄位」在 87 項測試全綠下發生，mock 全來自 golden fixture 抓不到
- 修正假綠燈：e2e `search()` helper 先清空並等提示出現再輸入（原本第二次呼叫會被前次結果假通過）；「window 耗盡」測試實際點進詳細頁；deep link 驗現行價；E3 逐列比對
- 修正後 14 個變異：13 個被殺。第一輪 3 個存活皆為**錨錯位置**而非等價：R2 測試資料帶 `overlap` flag 由另一條規則擋下、R3 事件欄例外在 golden 中無 `terminated` 且從未有價的形態、R6 清橫幅被秒回的 status handler 順手重繪遮蔽——各補針對性測試後被殺
- 唯一存活「核心載入不比序號」：載入開始時橫幅與詳細頁的重試鈕都已清除，UI 無並行載入路徑，屬等價變異；序號檢查保留作縱深防禦
- 未補：SW 行為 e2e（需啟用 SW 的獨立設定），列為後續

## C2 人工截圖審查

`scripts/measure_e7.mjs` 以真實 `data/` 產生，存於 `.ai-review/screenshots/`：11 個 golden 代號 × 桌面 1280／行動 390，加搜尋頁。

- [x] 臨床藥師目檢：**通過（2026-09-12，使用者）**——價格線只出現在有價區間、終止／暫停為區塊、空窗中斷、預告為虛線、恢復支付不與終止前價格相連
...
- **精簡 index（spec §16）的效益**：window 只留搜尋卡需要的欄位、省略預設值 → raw 30.26 → 25.04 MB、解析 −34%（無節流 260 → 171 ms），但 gzip 只少 6%（3.28 → 3.07 MB）；下載才是大頭，實際省約 0.3–0.4 s。屬 Phase 1 schema 變更，未執行。

### 判定

**使用者 2026-09-12 決定：MVP 接受現況**（「解析完成→可搜尋」428 ms、首次開詳細頁 1.1–1.5 s 兩項未達目標）。index 精簡列為後續選項，不擋 v0.1.0。
