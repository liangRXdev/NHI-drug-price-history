...
- **C4 搜尋**：比對為不分大小寫的子字串（代號另支援前綴）。`AC48092100` → 第一筆為該代號，即使另有 > 50 筆候選；`ac4809` → 含該代號；「撫緒」、`caremod`、`paroxetine` → 各自含該代號；符合項位於全資料第 50 筆之後仍可找到；終止品項預設由「顯示已終止支付品項」勾選框隱藏（2026-09-12 使用者改決策，見 spec §8.1），但代號完全相符者一律顯示、被隱藏筆數須提示、全部被隱藏時不得回報「查無」；勾選後終止品項不被排除；空白查詢 → 顯示提示、不列結果；不存在字串 → 空結果訊息；任何查詢 render ≤ 50 筆。
...
| v2.4 | C4：新增「顯示已終止支付品項」勾選框，預設隱藏現行 0 元品項（使用者 2026-09-12 決定；原「終止品項不被排除」改為勾選後才成立） | 使用者需求 |
