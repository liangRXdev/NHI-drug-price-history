...
> NHI-drug-price-history 功能規格 **v0.3.3**（2026-09-14 定稿；2026-09-16 實作期修訂，見 §12）
...
| fetch 開始 → 清單首次可捲動 | **< 600 ms**（v0.3.3 修訂；原 200 ms，見 §8.2）（CPU 4x throttle + Fast 4G，冷快取，不含 `meta.json`／字型） |
| 篩選／排序重繪 | **< 120 ms**（v0.3.3 修訂；原 50 ms，見 §8.2） |
...
### 8.2 門檻修訂（v0.3.3，2026-09-16）

原本的 200 ms／50 ms 是**在終點定義還含糊時訂下的**：v0.3 實作期把秒表停在 `innerHTML`
指派完成（DOM 寫入），量到 126 ms／13 ms 就宣告達標。`/codex-review` 指出那不是 §8.1 寫的
「**可捲動**」——DOM 寫好但瀏覽器還沒 layout 時，清單並不能捲。把終點改成「DOM ＋ layout 完成」
後重測，同一份程式碼是 **781 ms／123 ms**。

已做的優化：`.upcoming-row` 加 `content-visibility: auto` ＋ `contain-intrinsic-size`，
畫面外的列不必先做 layout，降為 **約 400 ms／約 87 ms**（同機多批中位數 399–408／86–90 ms）。

**剩餘差距不是本功能可以解掉的**：82 列的 layout 本身就要這麼多，再快必須動到
每列的 DOM 結構或改為虛擬捲動，屬另一個提案。因此依實測值加合理餘裕修訂門檻：

| 項目 | 原目標 | 實測（中位數） | 修訂後 | 餘裕 |
|---|---|---|---|---|
| fetch → 可捲動 | 200 ms | 約 400 ms | **600 ms** | 約 1.5× |
| 篩選／排序重繪 | 50 ms | 約 87 ms | **120 ms** | 約 1.4× |

**這是接受現況，不是宣告已優化。** 修訂門檻的目的是讓下一個人看到綠燈時，知道綠燈代表
「與 2026-09-16 的實測相當」，而不是「已達成原始設計目標」。

- 終點定義**不得**再退回 DOM 寫入以換取綠燈——那正是本次覆審抓到的問題
- 量測機噪音明顯（同一份程式碼不同批次的中位數落在 233–408 ms），引用時以「約 400 ms」理解
- 若日後 `drug_index.json` 的載入策略或分片方式改變（見 `spec-compare.md` §8 的同類問題），
  應一併重測本節並考慮把門檻收回
...
| v0.3.3 | 2026-09-16 | §8 效能門檻依實測修訂（200 → 600 ms、50 → 120 ms），並新增 §8.2 記錄原目標、未達原因與已做的優化 | `.ai-review/upcoming-acceptance.md` §1 |
